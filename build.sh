...
CLASSES="$OUT/classes"
DEX="$OUT/dex"
UNSIGNED="$OUT/HelloBYD-unsigned.apk"
KEYSTORE="$ROOT/.signing/hello-byd-debug.keystore"
APK="$ROOT/HelloBYD.apk"

for required in "$TOOLS/aapt2" "$TOOLS/d8" "$TOOLS/apksigner" "$ANDROID_JAR"; do
  if [[ ! -e "$required" ]]; then
    echo "Missing Android SDK component: $required" >&2
    echo "Install it with: sdkmanager 'platforms;$PLATFORM' 'build-tools;$BUILD_TOOLS'" >&2
    exit 1
  fi
done
...
javac -source 8 -target 8 -encoding UTF-8 \
  ...
  -d "$CLASSES" \
  "$ROOT/src/com/devywork/hellobyd/MainActivity.java"

"$TOOLS/d8" --lib "$ANDROID_JAR" --min-api 26 --output "$DEX" \
  "$CLASSES/com/devywork/hellobyd/MainActivity.class"
...
if [[ ! -f "$KEYSTORE" ]]; then
  keytool -genkeypair -noprompt \
    ...
    -alias androiddebugkey -dname "CN=Hello BYD, O=DEVYWORK, C=PE" \
    -keyalg RSA -keysize 2048 -validity 10000 >/dev/null 2>&1
fi

"$TOOLS/zipalign" -f 4 "$UNSIGNED" "$OUT/HelloBYD-aligned.apk"
"$TOOLS/apksigner" sign \
  --ks "$KEYSTORE" --ks-pass pass:android --key-pass pass:android \
  --out "$APK" "$OUT/HelloBYD-aligned.apk"
